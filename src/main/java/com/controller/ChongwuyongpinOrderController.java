...
//                            chongwuyongpinOrderEntity.setCreateTime(date);//时间
    ...
    */
     ...
     * 根据订单号获取订单详情（包含商品信息）
     */
    @RequestMapping("/detailByOrderNo/{orderNo}")
    public R detailByOrderNo(@PathVariable("orderNo") String orderNo, HttpServletRequest request){
        logger.debug("detailByOrderNo方法:,,Controller:{},,orderNo:{}",this.getClass().getName(),orderNo);
        
        // 根据订单号查询所有订单项
        List<ChongwuyongpinOrderEntity> orderList = chongwuyongpinOrderService.selectList(
            new EntityWrapper<ChongwuyongpinOrderEntity>()
        ...
        for(ChongwuyongpinOrderEntity order : orderList){
            ...
            // 订单基本信息
            detailMap.put("orderId", order.getId());
            detailMap.put("orderNo", order.getChongwuyongpinOrderUuidNumber());
            detailMap.put("buyNumber", order.getBuyNumber());
            detailMap.put("truePrice", order.getChongwuyongpinOrderTruePrice());
            detailMap.put("orderType", order.getChongwuyongpinOrderTypes());
            detailMap.put("paymentType", order.getChongwuyongpinOrderPaymentTypes());
            detailMap.put("insertTime", order.getInsertTime());
            detailMap.put("createTime", order.getCreateTime());
            
            // 查询商品信息
            ChongwuyongpinEntity chongwuyongpin = chongwuyongpinService.selectById(order.getChongwuyongpinId());
            if(chongwuyongpin != null){
                Map<String, Object> productInfo = new HashMap<>();
                productInfo.put("productId", chongwuyongpin.getId());
                productInfo.put("productName", chongwuyongpin.getChongwuyongpinName());
                productInfo.put("productUuidNumber", chongwuyongpin.getChongwuyongpinUuidNumber());
                productInfo.put("productPhoto", chongwuyongpin.getChongwuyongpinPhoto());
                productInfo.put("productType", chongwuyongpin.getChongwuyongpinTypes());
                productInfo.put("productOldMoney", chongwuyongpin.getChongwuyongpinOldMoney());
                productInfo.put("productNewMoney", chongwuyongpin.getChongwuyongpinNewMoney());
                productInfo.put("productContent", chongwuyongpin.getChongwuyongpinContent());
                detailMap.put("product", productInfo);
            }
            
            // 查询收货地址信息
            AddressEntity address = addressService.selectById(order.getAddressId());
            if(address != null){
                Map<String, Object> addressInfo = new HashMap<>();
                addressInfo.put("addressId", address.getId());
                addressInfo.put("receiverName", address.getAddressName());
                addressInfo.put("receiverPhone", address.getAddressPhone());
                addressInfo.put("receiverAddress", address.getAddressDizhi());
                detailMap.put("address", addressInfo);
            }
            ...
            if(yonghu != null){
                ...
                userInfo.put("userId", yonghu.getId());
                userInfo.put("userName", yonghu.getYonghuName());
                userInfo.put("userPhone", yonghu.getYonghuPhone());
                userInfo.put("userPhoto", yonghu.getYonghuPhoto());
                detailMap.put("user", userInfo);
            }
        ...
        result.put("totalItems", orderDetails.size());
